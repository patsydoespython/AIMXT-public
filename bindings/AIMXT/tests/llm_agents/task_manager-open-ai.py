...
from AIMXT import Agent, CoreAdmin, on_message
...
class TaskManager(CoreAdmin):
    ...
    async def get_best_agent_for_task(self, task: Task) -> str:
        ...
        prompt = f"""Given the following task and list of agents with their specialties, determine which agent is 
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": "You are an AI assistant that helps assign tasks to"
                            " the most suitable agent based on their specialties."},
                {"role": "user", "content": prompt}
            ]
        )

        best_agent = response.choices[0].message.content.strip()
        print(f"Best agent: {best_agent}")
        logger.info(f"OpenAI suggests assigning task '{task.description}' to agent '{best_agent}'")
        return best_agent
