...
from AIMXT.AIMXT import AgentCore, MessageHandler, Processor, AgentDefinition
from AIMXT.runner import AgentRunner
...
#
async def main():
    runner = AgentRunner(workspace_name="AIMXT-ai")
    runner.register_agent(Agent(name="AIMXT-ai-1",
                                responsibilities=["writer", "researcher"],
                                instructions=["How to use AI for Machine Learning"],
                                position="leader"))
    runner.register_agent(Agent(name="AIMXT-ai-2", responsibilities=["writer", "researcher"],
                                instructions=["How to use AI for Machine Learning"], position="leader"))
    runner.register_agent(Agent(name="AIMXT-ai-3", responsibilities=["writer", "researcher"],
                                instructions=["How to use AI for Machine Learning"], position="leader"))

    for i in range(4, 5):
        runner.register_agent(Agent(name=f"AIMXT-ai-{i}", responsibilities=["writer", "researcher"],
                                    instructions=["How to use AI for Machine Learning"], position="leader"))
